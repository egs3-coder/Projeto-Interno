...
static int card_value_for_score(int rank) {
    if (rank == 1) return 14;
...
void deck_init(Deck *deck) {
    ...
    for (int suit = 0; suit < 4; suit++) {
        for (int rank = 1; rank <= 13; rank++) {
            ...
            deck->cards[i].enhancement = ENHANCEMENT_NONE;
            i++;
        }
    }
    deck->top = 0;
...
Card deck_draw(Deck *deck) {
    Card empty = {0, 0, ENHANCEMENT_NONE};
    if (!deck_has_cards(deck)) return empty;
...
const char *enhancement_name(CardEnhancement enhancement) {
    switch (enhancement) {
        case ENHANCEMENT_BONUS: return "Bonus";
        case ENHANCEMENT_MULT: return "Multi";
        case ENHANCEMENT_LUCKY: return "Sorte";
        case ENHANCEMENT_GLASS: return "Vidro";
        case ENHANCEMENT_WILD: return "Coringa";
        default: return NULL;
    }
}

void print_card(Card card) {
    const char *enhancement = enhancement_name(card.enhancement);
    printf("%s de %s", rank_name(card.rank), suit_name(card.suit));
    if (enhancement) printf(" [%s]", enhancement);
}
...
static bool is_flush(Card selected[], int count) {
    ...
    for (int suit = 0; suit < 4; suit++) {
        bool ok = true;
        for (int i = 0; i < count; i++) {
            if (selected[i].enhancement == ENHANCEMENT_WILD) continue;
            if (selected[i].suit != suit) {
                ok = false;
                break;
            }
        }
...
static bool is_straight(Card selected[], int count) {
    ...
    for (int i = 1; i < count; i++) {
        if (ranks[i] != ranks[i - 1] + 1) normal = false;
    }
    if (normal) return true;

    int wheel[5] = {1, 2, 3, 4, 5};
    int original[MAX_PLAY];
    for (int i = 0; i < count; i++) original[i] = selected[i].rank;
...
HandEval evaluate_cards(Card selected[], int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        eval.rank_sum += card_value_for_score(selected[i].rank);
    }
